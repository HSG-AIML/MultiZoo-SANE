...
class DownstreamTaskLearner:
    ...
    def eval_dstasks(
        ...
        w_train, _ = trainset.__get_weights__(to_tensor=True)
        w_test, _ = testset.__get_weights__(to_tensor=True)
        if valset is not None:
             w_val, _ = valset.__get_weights__(to_tensor=True)
        ...
        for key in tqdm.tqdm(task_keys):
            ...
            if task_dx == "regression":
                ...
                reg, r2_train, r2_test, r2_val = self.compute_closed_form_solution(
                    z_train=z_train,
    ...
    def prepare_data(self, z_train, prop_train, z_test, prop_test, z_val=None, prop_val=None):
        # cast to tensor
    ...
    def eval_ood_dstask(
        ...
        # prepare embeddings
        print(f"Prepare embeddings")
        w_train = trainset.__get_weights__(to_tensor=True)
        z_train = self.map_embeddings(
        ...
        for datasetkey in testset_dict.keys():
            ...
            w_test = testset_dict[datasetkey].__get_weights__(to_tensor=True)
            z_test = self.map_embeddings(
        ...
        if valset is not None:
            w_val = valset.__get_weights__(to_tensor=True)
            z_val = self.map_embeddings(
                weights=w_val,
                model=model,
                batch_size=batch_size,
