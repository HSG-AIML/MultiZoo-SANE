...
class NN_tune_trainable_from_checkpoint(Trainable):
    def setup(self, config):
        ...
        elif sample_number is None and sample_epoch is None:
            # try load checkpoint from master path
            fname = str(checkpoint_master_path)
            if Path(fname).exists():
                pass
            else:
                raise RuntimeError(f"path to checkpoints does not exist. {fname}")
        else:
