...
from mzsane.sampling.halo import haloify, dehaloify

import ray

import copy

from mzsane.sampling.condition_bn import condition_checkpoints, check_equivalence
from mzsane.sampling.load_dataset import load_datasets_from_config
from mzsane.sampling.evaluate_ensemble import evaluate_ensemble
from mzsane.sampling.evaluate_single_model import evaluate_single_model
from mzsane.sampling.de_normalize import de_normalize_checkpoint
from mzsane.sampling.get_anchor_embeddings import (
    get_random_anchor_embeddings,
    get_anchor_embeddings,
)
from mzsane.sampling.sample_models import sample_models
...
def sample_model_evaluation_bootstrapped(
    ...
    anchor_ds_ref: Optional[Any] = None,  # reference to anchor dataset in ray object store, if available
    reference_dataset_ref: Optional[Any] = None,  # reference to reference dataset in ray object store, if available
) -> dict:
    """
    ...
    if anchor_ds_path is not None or anchor_ds_ref is not None:
    ...
    unique_layers = np.unique(anchor_pos[:, 1])
    logging.info(f"unique layers: {unique_layers}")
    for layer in unique_layers:
        layer_indices = np.where(anchor_pos[:, 1] == layer)
        layer_embeddings = anchor_z[:, layer_indices[0], :]
        layer_embeddings = layer_embeddings.flatten().cpu()
        layer_mean = float(layer_embeddings.mean().cpu().numpy())
        layer_std = float(layer_embeddings.std().cpu().numpy())
        layer_stats[str(layer)] = {"mean": layer_mean, "std": layer_std}
        logging.info(f"Layer {layer} embeddings mean: {layer_mean}")
        logging.info(f"Layer {layer} embeddings std: {layer_std}")
    ...
    for bdx in range(bootstrap_iterations):
        ...
        if bn_condition_iters > 0:
            ...
            checkpoints = condition_checkpoints(
                checkpoints, sample_config, bn_condition_iters, reference_dataset_ref=reference_dataset_ref
            )
        if check_equivalence(checkpoints[0], checkpoints[-1]):
        ...
        model_index_sorted, model_perf_sorted = bootstrap_model_evaluation(
            ...
            reference_dataset_ref=reference_dataset_ref,
        )
        ...
        checkpoints_sorted = [checkpoints[idx] for idx in model_index_sorted]

        logging.info(f"bootstrap iteration {bdx} - best models val acc {model_perf_best}")
        ...
        # link weight distributions to performance here
    ...
    if drop_samples_to_path is not None:
        ...
        for idx in range(len(checkpoints_sorted[:bootstrap_keep_top_n])):
            checkpoint = checkpoints_sorted[idx]
...
def bootstrap_model_evaluation(checkpoints, config, reference_dataset_path, reference_dataset_ref=None):
    """
    ...
    Args:
        ...
        reference_dataset_path: path to image dataset, which are used to evaluate the sampled models by for bootstrapping
        reference_dataset_ref: reference to reference dataset in ray object store, if available
    Returns:
    ...
    if reference_dataset_ref:
        print(f"load reference dataset from ray object store")
    ...
    if valloader is not None:
        ...
        print("use test set for evaluation")
    ...
    logging.info(f"model performance sorted: {model_perf}")
