...
from mzsane.datasets.dataset_auxiliaries import (
...
class ModelDatasetBaseEpochs(Dataset):
    ...
    def __init__(
            ...
            for edx in range(len(epoch_lst)):
                ...
                if ref_path is not None:
                    try:
                        ...
                        config_path = ref_path.joinpath("config.json")
                        ref_config = json.load(config_path.open("r"))
                else:
                    ref_config = None
        ...
        # three splits
            ...
            print(f"path list length: {len(self.path_list)}")
            if self.train_val_test == "train":
                idx1 = int(self.ds_split[0] * len(self.path_list))
                self.path_list = self.path_list[:idx1]
                ...
                print(f"val path list length: {len(self.path_list)}")
            elif self.train_val_test == "test":
                idx1 = int(self.ds_split[0] * len(self.path_list))
                idx2 = idx1 + int(self.ds_split[1] * len(self.path_list))
            ...
            for jjdx, eedx in enumerate(self.epochs[iidx]):
                ...
                if read_ggap:
                    try:
                        ...
                        gap = res_tmp["train_acc"] - res_tmp["val_acc"]
...
def read_properties_from_path(path, idx, idx_offset):
    ...
    try:
        ...
        try:
            ...
            results = json.load(fname.open("r"))
...
def load_checkpoints_remote(
    ...
    try:
        ...
        try:
            ...
            try:
                # try with conventional naming scheme
                chkpth = path.joinpath(f"checkpoint_{edx:04d}", "checkpoints")
                chkpoint = torch.load(str(chkpth), map_location=torch.device("cpu"))
            except FileNotFoundError:
                logging.debug("File not found. Trying alternative formatting.")
                # use other formatting
...
def load_checkpoint(
    ...
    try:
        ...
        try:
            ...
            logging.debug(f"File not found. try again with different formatting: {e}")
            for fmt in [f"checkpoint_{edx:02d}", f"checkpoint_{edx:04d}", f"checkpoint_{edx:06d}"]:
                try:
                    # use formatting option
                    chkpth = path.joinpath(fmt, "checkpoints")
                    # load chkpoint to cpu memory
                    chkpoint = torch.load(str(chkpth), map_location=torch.device("cpu"))
                    break  # exit the loop if checkpoint is successfully loaded
                except FileNotFoundError as e:
                    logging.debug(f"File not found. try again with different formatting: {e}")
    except Exception as e:
        logging.debug(f"error while loading {chkpth}: {e}")
