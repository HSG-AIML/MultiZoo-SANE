...
from mzsane.datasets.progress_bar import ProgressBar
...
class TwoViewSplit(torch.nn.Module):
    ...
    def _forward_permutation(self, ddx1, mdx1, p1):
        ...
        if len(ddx1.shape) == 2:
            return self._forward_copy(ddx1, mdx1, p1)
        ...
        if self.view_1_canon == True:
            perm_idx_1 = torch.tensor(0) # assign extra index in case of both canon view and ddxx1.shape[-3] == 1
        else:
            perm_idx_1 = perm_ids[0]
        if self.view_2_canon == True:
            perm_idx_2 = torch.tensor(0) # assign extra index in case of both canon view and ddxx1.shape[-3] == 1
        else:
...
class NoiseAugmentation(torch.nn.Module):
    ...
    def _forward_multiplicative(self, ddx, mdx, p, props=None):
        ddx = ddx * (1.0 + self.sigma * torch.randn(ddx.shape, device=ddx.device))
        if props is not None:
            return ddx, mdx, p, props
        return ddx, mdx, p

    def _forward_additive(self, ddx, mdx, p, props=None):
