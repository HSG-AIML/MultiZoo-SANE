...
from mzsane.git_re_basin.git_re_basin import PermutationSpec

from mzsane.datasets.dataset_tokens import DatasetTokens
from mzsane.datasets.augmentations import WindowCutter, CheckpointAugmentationPipeline
...
def prepare_datasets_for_multiple_architectures(configurations: List[Dict[str, Any]]):
    """
...
def preprocess_single_split(
    ...
    if permutation_number > 0:
        ...
        dataset.transforms = WindowCutter(windowsize=1000000000) # set windowsize very large, to infer positions)
        _, _, pos_tmp, _ = dataset.__getitem__(0)
