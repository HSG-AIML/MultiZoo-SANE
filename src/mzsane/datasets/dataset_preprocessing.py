...
from mzsane.datasets.dataset_tokens import DatasetTokens
...
def prepare_multiple_datasets(configurations: List[Dict[str, Any]]):
    ...
    for config in configurations:
        prepare_dataset(
            ...
            permutations_per_sample_train=config.get(
                "permutations_per_sample_train", 0
            ),
            ...
            clean_keys=config.get("clean_keys", False),
        )
...
def save_dataset(dataset, output_dir, num_workers=8):
    # Create a DataLoader
    ...
    for index, (ddx, mask, pos, props) in enumerate(tqdm(dataloader)):
...
def prepare_dataset(
    ...
    drop_pt_dataset: bool = True,
    clean_keys: bool = False,
):
    """
    Prepares a preprocessed dataset from token_dataset.
    Args:
...
def preprocess_single_split(
    ...
    """
    Loads a single split of the token dataset.
    Args:
        dataset_target_path: Path to the target dataset.
        zoo_path: Path to the zoo.
        epoch_list: List of epochs to use.
    ...
    if permutation_number > 0:
        ...
        dataset.transforms = CheckpointAugmentationPipeline(
            ...
            windowsize=1000000000,  # set windowsize very large, to infer positions
        )
        _, _, pos_tmp, _ = dataset.__getitem__(0)
        dataset.transforms = CheckpointAugmentationPipeline(
            perm_spec=permutation_spec,
            tokensize=dataset.tokensize,
            ignore_bn=ignore_bn,
        ...
        dataset.transforms = WindowCutter(windowsize=1000000000)
        _, _, pos_tmp, _ = dataset.__getitem__(0)
    ...
    info = {
        ...
        "windowsize": windowsize,
        "split": split,
        "max_positions": pos_tmp.max(dim=0).values.tolist(),
    }
