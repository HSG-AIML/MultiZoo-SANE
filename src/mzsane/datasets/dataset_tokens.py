...
from mzsane.datasets.dataset_epochs import ModelDatasetBaseEpochs
from mzsane.git_re_basin.git_re_basin import (
    PermutationSpec,
...
class DatasetTokens(ModelDatasetBaseEpochs):
    ...
    def __init__(
        ...
        # rectify checkpoint keys if necessary
        if clean_keys:
            logging.info("cleaning checkpoint keys")
            for idx in range(len(self.data)):
                for jdx in range(len(self.data[idx])):
                    self.data[idx][jdx] = clean_checkpoint_keys(
                        self.data[idx][jdx], "module.", ""
                    )
            print(self.data[0][0].keys())
    ...
    def __get_weights__(
        ...
        if self.mode == "vector":
            ...
            # tokenize data
            data_tmp = [tokenize_checkpoint(checkpoint=ddx, tokensize=self.tokensize) for ddx in data] 
            # split tokens and masks
            data_out = []
            mask_out = []
            pos_out = []
            for ddx in data_tmp:
                data_out.append(ddx[0])
                mask_out.append(ddx[1])
                pos_out.append(ddx[2])
            self.positions = pos_out[-1]
            if to_tensor:
                data_out = torch.stack(data_out)
                mask_out = torch.stack(mask_out)
            return data_out, mask_out
    ...
    def __getitem__(
        ...
        if self.mode == "vector":
            ...
            elif self.getitem == "loaded_model+props":
                model = NNmodule(self.reference_config)
                model.model.load_state_dict(ddx)
                if self.transforms:
                    return self.transforms(model, props)
                return model, props
    ...
    def normalize_checkpoints_separately(self):
        """
        l2-normalization of all checkpoints and layers, individually (note: no shared normalization coeff)
        Store normalization coefficients for each model separately.
        """
        logging.info("apply l2 normalization")
        # store per-layer l2 norms for each model
        layers = {}
        ...
        for idx in tqdm.tqdm(
            ...
            for jdx in range(len(self.data[idx])):
                ...
                for key in self.data[idx][jdx].keys():
                    if "weight" in key:
                        ...
                        # store the l2 norm
                        if key not in model_layers:
                            model_layers[key] = []
                        model_layers[key].append(l2w.item())
            ...
            layers[f'model_{idx}'] = model_layers
...
def clean_checkpoint_keys(checkpoint, source="module.", target=""):
    check_new = OrderedDict()
    for key in checkpoint.keys():
        key_new = key.replace(source, target)
        check_new[key_new] = checkpoint[key]
    return check_new
